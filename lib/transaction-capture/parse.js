...
// Indian bank SMS write numeric dates day-first ("05-07-26" is 5 July 2026),
// but the JS Date parser reads them month-first — so numeric dates are parsed
// explicitly as DD-MM-YY(YY). Month-name forms ("12-Jul-26") are unambiguous
// and go through the native parser.
const NUMERIC_DMY_RE = /^(\d{1,2})[-/](\d{1,2})[-/](\d{2,4})$/;

function parseLooseDate(text) {
  const numeric = text.match(NUMERIC_DMY_RE);
  if (numeric) {
...
function tryRegexParse(rawText, sourceChannel) {
  ...
  return {
    ...
    // null when the text carries no date — the ingest route then falls back to
    // the device's capture timestamp, which is when the transaction actually
    // happened (not when the server got around to receiving it).
    date: dateMatch ? parseLooseDate(dateMatch[1]) : null,
...
"""`;
  ...
  return {
    ...
    // null (rather than "now") when the LLM found no date, so the ingest route
    // falls back to the device's capture timestamp.
